...
% Establecer un umbral para considerar el determinante como "cero"
umbral = 1e-10; % Un valor pequeño, pero mayor que los errores de redondeo
%Cuando MATLAB realiza cálculos con números de punto flotante, pueden ocurrir pequeños errores de redondeo. 
%Esto significa que el determinante de una matriz que debería ser singular (no invertible) podría dar un valor muy pequeño en lugar de cero
if size(A, 1) == size(A, 2) && abs(A_det) > umbral
    A_inv = inv(A);
...
if size(B, 1)== size(B, 2) && abs(B_det) > umbral
    B_inv = inv(B);
